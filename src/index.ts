...
function json(value: unknown, init: ResponseInit = {}): Response {
  ...
  for (const [key, item] of Object.entries(CORS_HEADERS))
    ...
    return [
      {
        ...
        url,
        ...(asString(attachment.proxy_url)
          ? { proxyUrl: asString(attachment.proxy_url) }
          : {}),
        ...(asString(attachment.content_type)
          ? { contentType: asString(attachment.content_type) }
          : {}),
        ...(asNumber(attachment.width) !== undefined
...
function buildCapture(
  ...
  if (!channelId || !targetId || !message)
    throw new Error("Discord 没有提供被选中的消息");
  ...
  const authorName =
    ...
    asString(message.author_name);
  if (!authorId || !authorName) throw new Error("无法读取消息作者");
  const authorBot =
    author?.bot === true || Boolean(asString(message.webhook_id));

  const channel = interaction.channel;
  const channelType = asNumber(channel?.type);
  const threadId =
    channelType !== undefined && THREAD_CHANNEL_TYPES.has(channelType)
      ? channelId
      : undefined;
  ...
  return {
    ...
    ...(asString(message.edited_timestamp)
      ...
      : {}),
    ...(threadId && asString(channel?.name)
...
async function createHandoff(env: Env, payload: unknown): Promise<string> {
  ...
  await env.DB.prepare(
    ...
    .bind(
      ...
      now,
      now + handoffTtlSeconds(env) * 1_000,
    )
    .run();
  return token;
}
...
async function consumeHandoff(env: Env, token: string): Promise<Response> {
  ...
  if (!row)
    return json({ error: "handoff_not_found_or_expired" }, { status: 404 });
...
async function registerMessageCommand(env: Env): Promise<void> {
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Discord command registration failed: ${response.status} ${detail}`,
    );
  }
}
...
function authorizedSetup(request: Request, env: Env): boolean {
  ...
  return (
    ...
    header === `Bearer ${env.DISCORD_BOT_TOKEN}`
  );
}

function openPage(request: Request, env: Env, token: string): Response {
  if (!/^[A-Za-z0-9_-]{30,160}$/u.test(token))
    return html("<h1>链接无效</h1>", { status: 400 });
        ...
        components: [
          {
            type: 1,
            components: [
              { type: 2, style: 5, label: "打开资源库", url: openUrl },
            ],
          },
        ],
      },
  ...
  async fetch(
    ...
    if (url.pathname.startsWith("/open/") && request.method === "GET") {
      return openPage(
        request,
        env,
        decodeURIComponent(url.pathname.slice("/open/".length)),
      );
    }
